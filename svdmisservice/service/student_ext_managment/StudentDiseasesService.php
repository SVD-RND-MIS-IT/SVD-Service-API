...
require_once '../../model/student_ext_managment/StudentDiseasesManagement.php';
 ...
 */
$app->post('/student_diseases_register',  function() use ($app) {
	
            // check for required params
            verifyRequiredParams(array('stu_id', 'dis_id' ));
			...
			$dis_id = $app->request->post('dis_id');
			$dis_found_year = $app->request->post('dis_found_year');
         
           
            $studentDiseasesManagement = new StudentDiseasesManagement();
			$res = $studentDiseasesManagement->createStudentDiseases($stu_id, $dis_id, $dis_found_year,1);
			
            if ($res == CREATED_SUCCESSFULLY) {
                $response["error"] = false;
                $response["message"] = "student_diseases is successfully registered";
            } else if ($res == CREATE_FAILED) {
                $response["error"] = true;
                $response["message"] = "Oops! An error occurred while registereing student_diseases";
            } else if ($res == ALREADY_EXISTED) {
                $response["error"] = true;
                $response["message"] = "Sorry, this student_diseases already exist";
            }
            // echo json response
