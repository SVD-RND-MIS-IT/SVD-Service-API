...
require_once '../../model/student_ext_managment/DiseasesManagement.php';
...
$app->get('/diseases',  function() {
            ...
            $diseasesManagement = new DiseasesManagement();
			$res = $diseasesManagement->getAllDiseases();

            $response["error"] = false;
            $response["diseases"] = array();

            // looping through result and preparing diseases array
            while ($diseases = $res->fetch_assoc()) {
                $tmp = array();
				
                $tmp["dis_id"] = $diseases["dis_id"];
                $tmp["dis_name"] = $diseases["dis_name"];
                $tmp["dis_explanation"] = $diseases["dis_explanation"];
				$tmp["status"] = $diseases["status"];
