...
require_once '../../model/thurunu_saviya_managment/ProjectSupervisorManagement.php';
...
$app->post('/project_register',   function() use ($app) {
            ...
            verifyRequiredParams(array('pro_name', 'pro_supervisor_id' ));
			...
			$res = $projectManagement->createProject($pro_name, $pro_discription, $pro_year, $pro_PDF_path, $pro_supervisor_id, 1);
 ...
 * url /projects        
 */
$app->get('/projectSupervisors',  function() {
            global $user_id;
			
            $response = array();
			
            $projectSupervisorManagement = new ProjectSupervisorManagement();
			$res = $projectSupervisorManagement->getAllProjectSupervisors();

            $response["error"] = false;
            $response["projectSupervisors"] = array();

            // looping through result and preparing projects array
            while ($projectSupervisors = $res->fetch_assoc()) {
                $tmp = array();
				
                $tmp["tea_id"] = $projectSupervisors["tea_id"];
                $tmp["tea_full_name"] = $projectSupervisors["tea_full_name"];
				
                array_push($response["projectSupervisors"], $tmp);
            }
