...
require_once '../../model/prefect_managment/PrefectPossitionManagement.php';
...
/*
 * ------------------------ PREFECT_POSSITION TABLE METHODS ------------------------
 */
 
/**
 * Prefect Possition Registration
	 
 * method - POST
 * params - pos_name
 */
$app->post('/prefect_possition_register',  'authenticate', function() use ($app) {
            ...
            // reading post params
            $pos_name = $app->request->post('pos_name');
         
           
            $prefectPossitionManagement = new PrefectPossitionManagement();
			$res = $prefectPossitionManagement->createPrefectPossition($pos_name,$currunt_user_id);
			
            if ($res == CREATED_SUCCESSFULLY) {
                $response["error"] = false;
                $response["message"] = "Prefect possition is successfully registered";
            } else if ($res == CREATE_FAILED) {
                $response["error"] = true;
                $response["message"] = "Oops! An error occurred while registereing prefect possition";
            } else if ($res == ALREADY_EXISTED) {
                $response["error"] = true;
                $response["message"] = "Sorry, this prefect possition already exist";
            }
            // echo json response
