...
require_once '../../model/exam_managment/ResultManagement.php';
...
$app->post('/result_register', function() use ($app) {
			...
			$res = $resultManagement->createResult($stu_id, $exm_id, $year, $result, $stu_exm_index_number);
			
            if ($res == CREATED_SUCCESSFULLY) {
                $response["error"] = false;
                $response["message"] = "Result is successfully registered";
            } else if ($res == CREATE_FAILED) {
                $response["error"] = true;
                $response["message"] = "Oops! An error occurred while registereing Result";
            } else if ($res == ALREADY_EXISTED) {
                $response["error"] = true;
                $response["message"] = "Sorry, this Result already exist";
            }
            // echo json response
 ...
 * url /exam/:examName          
 */
$app->get('/exam/:examName',  function($exm_name) {
            global $currunt_user_id;
            $response = array();
